//! Mock NTAG424 transport for CI testing.
//!
//! Simulates the full NTAG424 card protocol: authentication (AES-EV2),
//! file settings read/write, key change, key version read, NDEF read/write
//! (both ISO and native commands), and GetVersion.
//!
//! The mock tracks card state (keys, key versions, NDEF content, file
//! settings) and validates every authenticated APDU's MAC and encryption.
//! This makes it suitable for integration-testing the burn/wipe/diagnose
//! lifecycle without physical hardware.
...
/// Fixed TI (transaction identifier) returned after authentication.
const TI: [u8; 4] = [0x08, 0x5B, 0xC9, 0x41];

/// Factory-default NDEF file settings (7 bytes, no SDM).
...
/// Mock NTAG424 card that validates the full authenticated protocol.
...
impl MockTransport {
    /// Create a blank (factory-default) card.
    pub fn new() -> Self {
        Self::with_state(
            [[0u8; 16]; 5],
            [0u8; 5],
    ...
    /// Create a card in a specific state.
    pub fn with_state(
        keys: [[u8; 16]; 5],
        key_versions: [u8; 5],
        ndef: Vec<u8>,
    ...
    /// Create a provisioned card with the given keys and SDM file settings.
    #[allow(dead_code)]
    pub fn provisioned(
        keys: [[u8; 16]; 5],
        key_versions: [u8; 5],
    ...
    /// Return the log of all APDUs received (each entry is one C-APDU).
    #[allow(dead_code)]
    pub fn apdu_log(&self) -> &[Vec<u8>] {
        &self.apdu_log
    }
    ...
    fn handle_auth_part1(&mut self, apdu: &[u8]) -> Response<Vec<u8>> {
        ...
        assert!(key_no < 5, "key number {key_no} out of range");
    ...
    fn handle_auth_part2(&mut self, apdu: &[u8]) -> Response<Vec<u8>> {
        ...
        if &ciphertext[16..] != &rotated_rnd_b {
            // Wrong key — the caller encrypted with a key that differs
            // from the stored key. Return authentication error so the
            // higher-level code can try a different key.
            return Self::response(Vec::new(), 0x91, 0xAE);
        }
    ...
    // ── GetVersion (plain mode, 3 chained frames) ───────────────────
    ...
    fn handle_get_file_settings_mac(&mut self, apdu: &[u8]) -> Response<Vec<u8>> {
        self.require_ndef_selected();
    ...
    fn handle_change_key(&mut self, apdu: &[u8]) -> Response<Vec<u8>> {
        ...
        let key_no = apdu[5] as usize;
        assert!(key_no < 5, "key number out of range");
    ...
    // ── ReadData / WriteData (native commands) ──────────────────────
    ...
    fn handle_iso_read_binary(&mut self, apdu: &[u8]) -> Response<Vec<u8>> {
        ...
        // P1 P2 encode a 15-bit big-endian offset (no short file ID mode).
        let offset = (u16::from(apdu[2]) << 8) | u16::from(apdu[3]);
        // Le: 0x00 means 256 bytes.
        let want = if apdu[4] == 0 { 256 } else { apdu[4] as usize };
        let start = offset as usize;
        let available = self.ndef.len().saturating_sub(start);
        let take = want.min(available);
        let data = if take > 0 {
            self.ndef[start..start + take].to_vec()
        } else {
            Vec::new()
        };
        Self::response(data, 0x90, 0x00)
    }
}
...
impl Transport for MockTransport {
    ...
    async fn transmit(&mut self, apdu: &[u8]) -> Result<Response<Self::Data>, Self::Error> {
        self.apdu_log.push(apdu.to_vec());

        let response = if apdu == NDEF_DF_SELECT {
            self.ndef_selected = true;
            self.ndef_ef_selected = false;
            Self::iso_ok()
        } else if apdu == NDEF_EF_SELECT {
            self.require_ndef_selected();
            self.ndef_ef_selected = true;
            Self::iso_ok()
        } else if apdu.len() >= 2 && apdu[0] == 0x00 && apdu[1] == 0xD6 {
            // ISOUpdateBinary (unauthenticated NDEF write)
            self.require_ndef_selected();
            assert!(
                self.ndef_ef_selected,
            ...
            self.ndef = data;
            Self::iso_ok()
        } else if apdu.len() >= 2 && apdu[0] == 0x00 && apdu[1] == 0xB0 {
            self.handle_iso_read_binary(apdu)
        } else if apdu.len() >= 2 && apdu[0] == 0x90 && apdu[1] == 0x71 {
            self.handle_auth_part1(apdu)
        } else if apdu.len() >= 2 && apdu[0] == 0x90 && apdu[1] == 0xAF {
            if self.pending_auth.is_some() {
                self.handle_auth_part2(apdu)
            } else {
                self.handle_get_version_continue()
            }
        } else if apdu.len() >= 2 && apdu[0] == 0x90 && apdu[1] == 0x60 {
            self.handle_get_version_start()
        } else if apdu.len() >= 6 && apdu[0] == 0x90 && apdu[1] == 0xF5 {
            // Differentiate plain (Lc=0x01) from MAC (Lc=0x09) by APDU length.
            if apdu.len() == 7 {
                self.handle_get_file_settings_plain(apdu)
            } else {
                self.handle_get_file_settings_mac(apdu)
            }
        } else if apdu.len() >= 2 && apdu[0] == 0x90 && apdu[1] == 0x5F {
            self.handle_change_file_settings(apdu)
        } else if apdu.len() >= 2 && apdu[0] == 0x90 && apdu[1] == 0xC4 {
            self.handle_change_key(apdu)
        } else if apdu.len() >= 2 && apdu[0] == 0x90 && apdu[1] == 0x64 {
            self.handle_get_key_version(apdu)
        } else if apdu.len() >= 2 && apdu[0] == 0x90 && apdu[1] == 0xAD {
            self.handle_read_data_plain(apdu)
        } else if apdu.len() >= 2 && apdu[0] == 0x90 && apdu[1] == 0x8D {
            self.handle_write_data_plain(apdu)
        } else {
            panic!("unexpected APDU: {:02X?}", apdu);
        };
...
// ── Crypto helpers (same algorithms as the NTAG424 card) ────────────
