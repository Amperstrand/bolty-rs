//! AES-128-CMAC (RFC 4493) via the RustCrypto `cmac` crate.
//!
//! Kept behind this thin wrapper (rather than using `cmac` directly at call
//! sites) so the `[u8; 16] -> &[u8] -> [u8; 16]` signature stays fixed for
//! the no_std crates and the RFC 4493 vectors below keep guarding the
//! integration.
